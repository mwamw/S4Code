...
def _status(engine, invocation):
    return CommandResult.info(engine.format_status_overview())
...
def register_builtin_commands(registry) -> None:
    ...
    registry.register(S4Command("clear", CommandKind.LOCAL, "Clear conversation history.", _clear, category="Transcript", keywords=("reset", "history")))
    registry.register(
        S4Command(
            "compact",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("checkpoints", CommandKind.LOCAL, "List restorable conversation checkpoints.", _checkpoints, category="Sessions", keywords=("restore", "save")))
    registry.register(
        S4Command(
            "rewind",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("timeline", CommandKind.LOCAL, "Show session checkpoints and recent trace timeline.", _timeline, category="Sessions", keywords=("history", "trace")))
    registry.register(
        S4Command(
            "tools",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("files", CommandKind.LOCAL, "List project files.", _files, usage="[path]", category="Workspace", keywords=("repo", "tree")))
    registry.register(S4Command("diff", CommandKind.LOCAL, "Show git diff for the current repository.", _diff, usage="[target]", category="Workspace", keywords=("git", "changes"), priority=88))
    registry.register(S4Command("review", CommandKind.WORKFLOW, "Run a code review workflow against the current diff.", _review, usage="[target]", category="Workflows", keywords=("bugs", "regressions", "review"), priority=87))
    registry.register(S4Command("commit", CommandKind.WORKFLOW, "Draft a commit proposal from the current diff.", _commit, category="Workflows", keywords=("git", "message")))
    registry.register(S4Command("tasks", CommandKind.LOCAL, "List structured and background tasks.", _tasks, category="Runtime", keywords=("background", "jobs", "long-running"), priority=94))
    registry.register(
        S4Command(
            "task",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("agents", CommandKind.LOCAL, "List runtime agents.", _agents, category="Runtime", keywords=("workers", "handles")))
    registry.register(
        S4Command(
            "agent",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("hooks", CommandKind.LOCAL, "List installed hooks/guardrails.", _hooks, category="Debug", keywords=("guardrails", "hooks")))
    registry.register(
        S4Command(
            "confirm",
            CommandKind.LOCAL,
    ...
    registry.register(S4Command("sidebar", CommandKind.LOCAL, "Show or hide the right-side info panel.", _sidebar, usage="[show|hide]", category="Settings", keywords=("panel", "ui")))
    registry.register(S4Command("exit", CommandKind.LOCAL, "Exit the current S4Code session.", _exit, aliases=("quit", "q"), category="General", keywords=("quit", "close")))

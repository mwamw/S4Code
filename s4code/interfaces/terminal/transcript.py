...
class S4TranscriptState:
    ...
    def _remove_card(self, card_id: str) -> None:
        ...
        stale_tool_ids = [
            tool_id
            for tool_id, value in self._tool_card_ids.items()
            if value == card_id
        ]
    ...
    def _ensure_content_card(self) -> TranscriptCard:
        ...
        metrics = (
            self._round_metrics_for_card(round_card)
            if round_card is not None
            else self._new_round_metrics()
        )
        card = self.append_card(
            "assistant",
            "Model Response",
    ...
    def _sync_round_message_footer(
        self, round_number: int, metrics: dict[str, Any]
    ) -> None:
    ...
    def consume_event(self, event: dict[str, Any]) -> None:
        ...
        if event_type == "thinking_delta":
            ...
            if not delta:
                return
            if (
                self._current_round is None
                or self._current_round.thinking_card_id is None
            ) and not delta.strip():
                return
        ...
        if event_type == "text_delta":
            ...
            if (
                self._current_round is None
                or self._current_round.content_card_id is None
            ) and not delta.strip():
                return
        ...
        if event_type == "tool_call":
            ...
            tool_id = str(
                event.get("tool_id") or f"tool-{len(self._tool_card_ids) + 1}"
            )
            ...
            if round_card is not None:
                ...
                tools_used = {
                    str(item) for item in list(metrics.get("tools_used") or []) if item
                }
        ...
        if event_type == "tool_result":
            ...
            if round_state is not None:
                ...
                if round_card is not None:
                    ...
                    metrics["running_tools"] = max(
                        int(metrics.get("running_tools") or 0) - 1, 0
                    )
                    metrics["local_tool_seconds"] = (
                        float(metrics.get("local_tool_seconds") or 0.0) + duration
                    )
                    if status == "error":
                        metrics["tool_errors"] = (
                            int(metrics.get("tool_errors") or 0) + 1
                        )
                    elif status == "pending":
                        metrics["tool_pending"] = (
                            int(metrics.get("tool_pending") or 0) + 1
                        )
                    ...
                    if isinstance(diff_payload, dict):
                        ...
                        label = str(
                            diff_payload.get("relative_path")
                            or diff_payload.get("file_path")
                            or ""
                        ).strip()
                        if label:
                            changed.add(label)
        ...
        if event_type == "round_metrics":
            ...
            for key, value in payload.items():
                if key in {"files_changed", "tools_used"}:
                    existing = {
                        str(item)
                        for item in list(metrics.get(key) or [])
                        if str(item).strip()
                    }
                    incoming = {
                        str(item) for item in list(value or []) if str(item).strip()
                    }
            ...
            if (
                ...
                and self._current_round.number == round_number
            ):
                self._refresh_live_round_body()
            else:
        ...
        if event_type == "runtime_snapshot":
            ...
            existing = (
                self.find_card(self._runtime_card_id) if self._runtime_card_id else None
            )
            if existing is None:
                card = self.append_card(
                    "runtime",
        ...
        if event_type == "compaction_result":
            ...
            existing = (
                self.find_card(self._compaction_card_id)
                if self._compaction_card_id
                else None
            )
        ...
        if event_type == "final":
            ...
            if round_number is not None:
                self._append_final_summary_cards(
                    round_number=int(round_number), metrics=metrics, content=content
                )
            return
    ...
    @staticmethod
    def _format_error_body(event: dict[str, Any]) -> str:
        lines = [str(event.get("error") or "Unknown error").strip()]
        fields = (
    ...
    def _add_checkpoint_to_card(
        self, card: TranscriptCard, checkpoint: dict[str, Any]
    ) -> None:
    ...
    def _find_checkpoint_target(
        self, checkpoint: dict[str, Any]
    ) -> Optional[TranscriptCard]:
    ...
    def _format_tool_result_body(self, event: dict[str, Any]) -> str:
        ...
        if isinstance(structured_data, dict):
            ...
            if task_id and status in {"running", "queued", "waiting"}:
                ...
                command = (
                    str(structured_data.get("command") or "").replace("\n", " ").strip()
                )
                if description:
                    lines.append(
                        f"Purpose: {self._summarize_scalar(description, max_chars=140)}"
                    )
                elif command:
                    lines.append(
                        f"Command: {self._summarize_scalar(command, max_chars=140)}"
                    )
    ...
    def _extract_tool_diff(self, structured_data: Any) -> Optional[dict[str, Any]]:
        ...
        return {
            "unified": unified,
            "file_path": str(
                diff_payload.get("file_path") or structured_data.get("file_path") or ""
            ).strip(),
    ...
    def _format_runtime_snapshot(self, snapshot: Any) -> str:
        ...
        if tasks or background_tasks:
            for item in tasks[:6]:
                lines.append(
                    f"- {item.get('task_id') or '-'} | {item.get('status') or '-'} | {item.get('title') or '-'}"
                )
            for item in background_tasks[:6]:
                ...
                if stdout_tail:
                    lines.append(
                        f"  stdout: {self._summarize_scalar(stdout_tail, max_chars=160)}"
                    )
                if stderr_tail:
                    lines.append(
                        f"  stderr: {self._summarize_scalar(stderr_tail, max_chars=160)}"
                    )
        else:
            lines.append("- none")
        if context:
    ...
    def _format_interruption_body(
        ...
        if interaction_type == "enter_plan_mode":
            ...
            if allowed_actions:
                ...
                lines.extend(
                    f"- {self._summarize_scalar(item, max_chars=120)}"
        ...
        risk_line = self._pending_risk_summary(
            tool_name, payload.get("tool_args") or {}
        )
        if risk_line:
        ...
        lines.append(
            "- Use `/confirm remember` to allow a matching action for this session."
        )
    ...
    def _format_round_metrics_line(
        self, metrics: dict[str, Any], *, outcome: str
    ) -> str:
        items: list[str] = []
    ...
    def _format_active_round_body(
        self, started_at: float, *, now: float, metrics: Optional[dict[str, Any]] = None
    ) -> str:
        lines = [f"Elapsed: {self._format_duration(now - started_at)}"]
        metrics_payload = metrics or {}
        metrics_line = self._format_round_metrics_line(
            metrics_payload, outcome="running"
        )
        if metrics_line:
    ...
    def _format_round_details(self, metrics: dict[str, Any]) -> list[str]:
        ...
        tools_used = [
            str(item).strip()
            for item in list(metrics.get("tools_used") or [])
            if str(item).strip()
        ]
        if tools_used:
    ...
    def _append_final_summary_cards(
        self, *, round_number: int, metrics: dict[str, Any], content: str
    ) -> None:
        tools_used = [
            str(item).strip()
            for item in list(metrics.get("tools_used") or [])
